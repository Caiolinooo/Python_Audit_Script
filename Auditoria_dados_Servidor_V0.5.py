...
for cliente in os.listdir(pasta_raiz):
    ...
    if os.path.isdir(caminho_cliente):
        ...
        dados_excel.append({
            ...
            'WorkspaceData': 'Sim' if verificar_pasta(caminho_cliente, 'WorkspaceData') else 'Não'
        })
...
with pd.ExcelWriter(nome_arquivo, engine='xlsxwriter') as writer:
    ...
    for row_num, row in df.iterrows():
        if row['Cliente'].startswith('  - '):
            ...
            worksheet.write(row_num + 1, 4, row['Caminho'], subpasta_format)
            worksheet.write(row_num + 1, 5, row['WorkspaceData'], subpasta_format)
        else:
            ...
            worksheet.write(row_num + 1, 4, row['Caminho'], cliente_format)
            worksheet.write(row_num + 1, 5, row['WorkspaceData'], cliente_format)
    ...
    worksheet.set_column('E:E', 60)  # Caminho
    worksheet.set_column('F:F', 15)  # WorkspaceData
