...
class AuditoriaServidor:
    ...
    def executar_auditoria(self):
        ...
        for cliente in os.listdir(self.pasta_raiz):
            caminho_cliente = os.path.join(self.pasta_raiz, cliente)
            if os.path.isdir(caminho_cliente):
                clientes_processados.add(cliente)
                tarefas.append((caminho_cliente, cliente, False))
                
                subpastas = [
                    (os.path.join(caminho_cliente, subpasta), subpasta, True)
                    for subpasta in os.listdir(caminho_cliente)
                    if os.path.isdir(os.path.join(caminho_cliente, subpasta))
                ]
                tarefas.extend(subpastas)

        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = [executor.submit(self.processar_pasta_paralelo, tarefa) 
                      for tarefa in tarefas]
            
            for future in tqdm(as_completed(futures), 
                             total=len(futures), 
                             desc="Processando pastas"):
                if resultado := future.result():
                    self.dados_excel.append(resultado)
        
        self.dados_excel.sort(key=lambda x: (
            x['Cliente'] if not x['Cliente'].startswith(' - ') else x['Cliente'][3:],
            x['Cliente'].startswith(' - ')
        ))
