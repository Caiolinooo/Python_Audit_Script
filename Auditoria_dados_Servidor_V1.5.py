...
# Solicitar a unidade que deve ser auditada
pasta_raiz = input("Insira o caminho da pasta que deseja auditar: ")

# Solicitar o local de saída do relatório
local_saida = input("Insira o caminho onde deseja salvar o relatório: ")
...
for cliente in os.listdir(pasta_raiz):
    ...
    if os.path.isdir(caminho_cliente):
        ...
        for subpasta in os.listdir(caminho_cliente):
            ...
            if os.path.isdir(caminho_subpasta):
                ...
                subpastas.append({
                    ...
                    'scene': 'Sim' if arquivos_subpasta['.scene'] else 'Não',
        ...
        # Verificar se há pasta WorkspaceData
        if verificar_pasta(caminho_cliente, 'WorkspaceData'):
            ...
            data_criacao_subpastas = data_criacao_cliente
        # Caso não haja, verificar se há pasta Revisions
        elif verificar_pasta(caminho_cliente, 'Revisions'):
            data_criacao_cliente = obter_data_criacao_subpasta(caminho_cliente, 'Revisions')
            data_criacao_subpastas = data_criacao_cliente
        # Caso não haja nenhuma, usar a data padrão da pasta
        else:
        ...
        for sub in subpastas:
            dados_excel.append({
                'Cliente': f"  - {sub['Nome']}",
                'Data Criação': data_criacao_subpastas,
...
with pd.ExcelWriter(caminho_arquivo, engine='xlsxwriter') as writer:
    ...
    for row_num, row in df.iterrows():
        if row['Cliente'].startswith('  - '):
            ...
            worksheet.write(row_num + 1, 4, row['scene'], subpasta_format)
            ...
            worksheet.write(row_num + 1, 4, row['scene'], cliente_format)
    ...
    worksheet.set_column('E:E', 10)  # scene
