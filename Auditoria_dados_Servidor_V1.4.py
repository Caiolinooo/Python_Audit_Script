...
for dependencia in dependencias:
    install_and_import(dependencia)
...
# Localizar a pasta Clientes
pasta_clientes = os.path.join(pasta_raiz, 'Clientes')

if os.path.exists(pasta_clientes):
    print("Pasta 'Clientes' encontrada.")
    
    # Auditoria
    for cliente in tqdm(os.listdir(pasta_clientes), desc="Escaneando pastas"):
        caminho_cliente = os.path.join(pasta_clientes, cliente)
        if os.path.isdir(caminho_cliente):
            print(f"Processando cliente: {cliente}")
            
            dados_excel.append({
                'Cliente': cliente,
                'Data Criação': obter_data_criacao(caminho_cliente),
                'Tamanho Total (GB)': round(calcular_tamanho_pasta(caminho_cliente) / (1024 * 1024 * 1024), 0),
                **{tipo[1:]: 'Não' for tipo in tipos_arquivos}
            })
            
            # Verificar arquivos no cliente
            arquivos_cliente = verificar_arquivos(caminho_cliente)
            for tipo in tipos_arquivos:
                if arquivos_cliente[tipo]:
                    dados_excel[-1][tipo[1:]] = 'Sim'
            ...
            for subpasta in os.listdir(caminho_cliente):
                ...
                if os.path.isdir(caminho_subpasta):
                    ...
                    dados_excel.append({
                        'Cliente': f"  - {subpasta}",
                        'Data Criação': data_criacao_subpasta,
                        'Tamanho Total (GB)': tamanho_subpasta,
                        **{tipo[1:]: 'Sim' if arquivos_subpasta[tipo] else 'Não' for tipo in tipos_arquivos}
                    })
else:
    print("Pasta 'Clientes' não encontrada.")
...
if dados_excel:
    ...
    nome_arquivo = f"Auditoria_{nome_unidade}_{os.path.basename(pasta_raiz)}.xlsx"
    ...
    with pd.ExcelWriter(caminho_arquivo, engine='xlsxwriter') as writer:
        ...
        for row_num, row in df.iterrows():
            if row['Cliente'].startswith('  - '):
                ...
                caminho_cliente = os.path.join(pasta_clientes, row['Cliente'])
